# 多个EKS集群使用（推荐方式）
module "development_vpc_multi" {
  source = "./modules/vpc"

  vpc_cidr              = "172.21.0.0/16"
  public_subnet_cidrs   = ["172.21.1.0/24", "172.21.2.0/24", "172.21.3.0/24"]
  private_subnet_cidrs  = ["172.21.101.0/24", "172.21.102.0/24", "172.21.103.0/24"]
  availability_zones    = ["us-east-1a", "us-east-1b", "us-east-1c"]
  environment           = "development"
  bastion_instance_type = "t3.micro"
  ssh_public_key        = "ssh-rsa  pengchaoma@Pengchaos-MacBook-Pro.local"

  # 单个集群
  eks_cluster_name = "my-eks-cluster"
  # 多个集群
  #eks_cluster_names     = ["dev-eks-cluster", "staging-eks-cluster", "prod-eks-cluster"]
  region               = "us-east-1"
  create_vpc_endpoints = true
}